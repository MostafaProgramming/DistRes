...
After running `launch_live_ui.ps1`, use:

| View | URL | Purpose |
| --- | --- | --- |
| Live Client UI | `http://127.0.0.1:8200` | Log in as clients, read/write the shared resource, and receive subscriber notifications. |
| Server Dashboard | `http://127.0.0.1:8200/server.html` | Read-only monitor showing connected clients, active locks, waiting requests, subscribers, and server events. |
| Replay Visualiser | `http://localhost:8100` | Replay evidence generated by the automated demo logs. |
...
- read-only server dashboard for live demonstration of server coordination

## Project Structure

| Path | Role |
| --- | --- |
| `DistResServer.cpp` | C++ server node: accepts clients, authenticates users, controls resource access, and publishes updates. |
| `DistResClient.cpp` | C++ client node: connects over TCP, retries connection, sends read/write commands, and listens for events. |
| `DistResCommon.h` | Shared TCP helpers and `WriterFairRWLock` synchronisation logic. |
| `CredentialsDatabase.txt` | Server-hosted user credential records used by `UserStore`. |
| `ProductSpecification.txt` | Server-hosted shared distributed resource. |
| `live_frontend/` | Live browser client and read-only server dashboard used in the demo. |
| `frontend/` | Replay visualiser for automated demo evidence. |
| `reports/` | Coursework design/implementation diagrams and report assets. |
| `build.ps1`, `run_demo.ps1` | Build and automated evidence demo scripts. |

## Technology Choices

The repository uses several languages because each one has a specific role:

| Language | Why it is used |
| --- | --- |
| C++ | Core distributed system implementation: TCP server/client, threads, locks, authentication, shared resource access, and publish-subscribe. |
| PowerShell | Windows build and demonstration scripts. |
| Python | Small local bridge for the browser demo and automated evidence generation. It does not replace the C++ server logic. |
| HTML/CSS/JavaScript | Browser UI, replay visualiser, and read-only server dashboard for demonstration clarity. |

The assessed distributed-system behaviour is implemented in C++. The other languages support building, demonstrating, and explaining the system.
